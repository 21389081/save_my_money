...
import { BarChart3, BookOpen, Home, Plus, Settings, WalletCards, Clock } from 'lucide-react';
...
const navigation = [
    ...
    { href: '/exchange', label: '最新匯率', icon: Clock },
    ...
    return (
        <div className='min-h-screen bg-white md:grid md:grid-cols-[248px_1fr]'>
            <aside className='fixed inset-y-0 left-0 z-30 hidden w-[248px] border-r border-line bg-white px-5 py-7 md:flex md:flex-col'>
                ...
                <nav aria-label='桌面主要導航' className='mt-10 space-y-2'>
                    {navigation.map((item) => {
                        const active =
            ...
            <nav
                aria-label='手機主要導航'
                className='safe-bottom fixed inset-x-0 bottom-0 z-30 grid grid-cols-5 gap-0 border-t border-line bg-white/95 px-1 pt-2 backdrop-blur md:hidden'
            >
                {navigation.map((item) => {
                    const active =
                        item.href === '/' ? pathname === '/' : pathname.startsWith(item.href);
