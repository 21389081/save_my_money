...
import { calculateBalance, calculateBudgetProgress, getMonthlySummary } from '@/lib/finance';
import { formatCurrency } from '@/lib/format';
import { toMonthKey } from '@/lib/date';

export default function DashboardPage() {
    const { state } = useApp();
    const ledger = state.ledgers.find((item) => item.id === state.currentLedgerId);
        ...
        <>
            ...
            <section className='mt-9'>
                <div className='mb-3 flex items-center justify-between'>
                    <h2 className='text-lg font-bold'>錢都去哪了？</h2>
                    <Link
                        href='/stats'
