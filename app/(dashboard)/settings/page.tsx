...
export default function SettingsPage() {
  ...
  const reset = () => {
    if (window.confirm("確定要清除所有本機變更，恢復示範資料嗎？")) {
      resetData();
    }
  };
    ...
    <>
      ...
      <section className="mt-8 space-y-3">
        <button
          type="button"
          onClick={reset}
          className="flex min-h-14 w-full items-center gap-3 rounded-2xl border border-line px-5 text-left text-sm font-bold transition hover:bg-surface"
        >
          <RotateCcw size={19} className="text-primary-strong" />
          清除並重建示範資料
        </button>
        <button
          type="button"
